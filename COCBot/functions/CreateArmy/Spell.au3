...
; Parameters ....:
; Return values .: None
; Author ........: ProMac ( 08-2015) , N2delay (2016)
; Modified ......:
		...
		If $CurSFactory < $TotalSFactory Then ;  If not full the Spell Factory then try to make spells
			...
			If $numFactoryDarkSpellAvaiables = 1 And ($PoisonSpellComp > 0 Or $HasteSpellComp > 0) Then
				...
				If isDarkSpellFactory() Then
					...
					If $EarthQSpellComp > 0 Then ; Haste Spells
						$TempEarthQSpell = Number(getBarracksTroopQuantity(175 + 107 * 1, 296 + $midOffsetY))
						Local $earthQSpell = $EarthQSpellComp - ($CurEarthSpell + $TempEarthQSpell)
						If $debugSetlog = 1 Then SetLog("Making Haste Spell: " & $earthQSpell)
						If _sleep($iDelayTrain2) Then Return
						If $TempEarthQSpell = 0 Then
							If _sleep($iDelayTrain2) Then Return
							If _ColorCheck(_GetPixelColor(347, 375 + $midOffsetY, True), Hex(0xFFFFFF, 6), 20) = False Then ; black pixel in number 5
								setlog("Not enough Elixir to create Spell", $COLOR_RED)
								Return
							ElseIf _ColorCheck(_GetPixelColor(200, 346 + $midOffsetY, True), Hex(0x414141, 6), 20) Then
								setlog("Spell Factory Full", $COLOR_RED)
								Return
							Else
								If $earthQSpell > 0 Then
									GemClick(325, 354 + $midOffsetY, $earthQSpell, $iDelayTrain7, "#0290")
									SetLog("Created " & $earthQSpell & " EarthQuake Spell(s)", $COLOR_BLUE)
								EndIf
							EndIf
						Else
